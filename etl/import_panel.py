...
def fetch_firm_id_map(conn, tickers: List[str]) -> Dict[str, int]:
    if not tickers:
        return {}
    sql = text("SELECT firm_id, ticker FROM dim_firm WHERE ticker IN :tickers")
    sql = sql.bindparams(bindparam("tickers", expanding=True))
    result = conn.execute(sql, {"tickers": tickers})
    return {r.ticker.upper(): int(r.firm_id) for r in result}
...
def upsert_many(conn, table: str, cols: List[str], rows: List[Tuple[Any, ...]]) -> int:
    ...
    data = [dict(zip(cols, [_fix_nan(x) for x in row])) for row in rows]

    result = conn.execute(text(sql), data)
    return result.rowcount
    
def get_data_source_id(conn, source_name: str) -> int:
    sql = text("SELECT source_id FROM dim_data_source WHERE source_name=:name")
    row = conn.execute(sql, {"name": source_name}).fetchone()
    if not row:
        raise SystemExit(f"source_name '{source_name}' not found.")
...
def get_snapshot_id(conn, source_id: int, fiscal_year: int, version_tag: str) -> int:
    sql = text("""
        SELECT snapshot_id, snapshot_date
            ...
            WHERE source_id=:source_id AND fiscal_year=:fiscal_year AND version_tag=:version_tag
...
def main():
    ...
    ap.add_argument("--excel", default="Final Gộp 39 trường dữ liệu (FINAL 3).xlsx")
