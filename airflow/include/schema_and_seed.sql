...
CREATE TABLE IF NOT EXISTS `fact_ownership_year` (
  ...
  CONSTRAINT `fk_fact_ownership_year_snapshot_id` FOREIGN KEY (`snapshot_id`)
    ...
    ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `ck_fact_ownership_year_managerial_inside_own` CHECK (`managerial_inside_own` IS NULL OR (`managerial_inside_own`>=0 AND `managerial_inside_own`<=1)),
  CONSTRAINT `ck_fact_ownership_year_state_own` CHECK (`state_own` IS NULL OR (`state_own`>=0 AND `state_own`<=1)),
  CONSTRAINT `ck_fact_ownership_year_institutional_own` CHECK (`institutional_own` IS NULL OR (`institutional_own`>=0 AND `institutional_own`<=1)),
  CONSTRAINT `ck_fact_ownership_year_foreign_own` CHECK (`foreign_own` IS NULL OR (`foreign_own`>=0 AND `foreign_own`<=1))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `fact_innovation_year` (
  ...
  CONSTRAINT `fk_fact_innovation_year_evidence_source_id` FOREIGN KEY (`evidence_source_id`)
    REFERENCES `dim_data_source` (`source_id`)
    ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `ck_fact_innovation_year_product_innovation` CHECK (`product_innovation` IS NULL OR `product_innovation` IN (0,1)),
  CONSTRAINT `ck_fact_innovation_year_process_innovation` CHECK (`process_innovation` IS NULL OR `process_innovation` IN (0,1))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
INSERT INTO dim_exchange (exchange_id, exchange_code, exchange_name) VALUES
...
INSERT INTO dim_data_source (source_id, source_name, source_type, provider, note) VALUES
  (1,'MarketData_Default','market','Reference','Seed market data source'),
  (2,'BCTC_Audited','financial_statement','Company/Exchange','Audited financial statements'),
  (3,'FiinPro_Ownership','ownership','FiinPro','Ownership report'),
  (4,'AnnualReport','text_report','Company','Annual report / disclosures')
ON DUPLICATE KEY UPDATE source_type=VALUES(source_type), provider=VALUES(provider), note=VALUES(note);